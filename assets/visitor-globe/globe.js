 ...
 *   - the current viewer is geolocated from their IP and adds one increment to
 *     a per-country counter, which is what the next snapshot picks up.
 *
...
(function () {
  ...
  var DEFAULTS = {
    ...
    counter: null,            // {base, namespace, prefix} — where visits are tallied
  ...
  // Read side: a same-origin snapshot regenerated by a scheduled job, shaped
  //   { cells: { "CN": { n: 42, lat: 35.9, lon: 104.2, l: "China" }, ... } }
  // Write side: one increment-only counter per country, so a hostile visitor
  // can at worst inflate their own bucket — never erase the map.
  ...
  function VisitorGlobe(options) {
    ...
    function updateCaption() {
      ...
      var here = state.you &&
        [state.you.city, state.you.country].filter(Boolean).join(", ");
      var parts = [];
    ...
    // Tally this visit and reflect it immediately, so a viewer whose country is
    // not in the latest snapshot still sees their region light up.
    function contribute(g) {
      ...
      var code = g.cc;
      if (code) {
        var known = cells[code];
        cells[code] = {
          n: (known ? +known.n || 0 : 0) + (fresh || !known ? 1 : 0),
          lat: known ? known.lat : g.lat,
          lon: known ? known.lon : g.lon,
          l: (known && known.l) || g.country || code
        };
      }
      rebuildCells();

      if (!fresh) return;
      var url = countUrl(cfg.counter, code);
      if (!url) return;
      fetch(url, { mode: "cors" }).then(function () {
